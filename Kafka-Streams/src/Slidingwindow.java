...
public class SlidingWindow {
    ...
    public static void main(String[] args) {
        ...
        streamsConfiguration.put(StreamsConfig.APPLICATION_ID_CONFIG, "Sliding");
        ...
        KStream<String, String> SLstream = initialstream
                .flatMapValues(value -> Arrays.asList(value.toLowerCase().split("\\W+")));

        KGroupedStream<String, String> SLgroupedStream = SLstream
                .groupBy((key, word) -> word, Grouped.with(Serdes.String(), Serdes.String()));

        KTable<Windowed<String>, Long> SLtable = SLgroupedStream
                .windowedBy(TimeWindows.of(windowSizeMs).advanceBy(advanceMs))
                .count();
        SLtable
                .toStream()
                .selectKey((key, word) -> key.key())
                .to("slidingoutputtopic", Produced.with(Serdes.String(), Serdes.Long()));

        SLgroupedStream
                .windowedBy(TimeWindows.of(windowSizeMs).advanceBy(advanceMs))
                .count(Materialized.<String, Long, WindowStore<Bytes, byte[]>>as("sliding-windowed-count")
                        .withValueSerde(Serdes.Long())
                        .withKeySerde(Serdes.String()))
                .toStream()
                .to("slidingstatestoretopic", Produced.with(windowSerde, Serdes.Long()));
